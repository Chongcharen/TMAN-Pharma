...
namespace OneP.InfinityScrollView
{
	...
	public class InfinityScrollView : MonoBehaviour {
		...
		public float itemGrid=100; // size of an item 
		...
		public int itemSpace = 0;//space size
		...
		public void Setup(){
			...
			itemSize = prefab.GetComponent<RectTransform> ().rect.height;
			itemSpace = 0;
			if (type == InfinityType.Vertical) {
				int totalHeight = (int)(((totalNumberItem + list_skip_Index.Count) * itemSize)+((totalNumberItem)*itemSpace));
		...
		private int GetCurrentIndex(){
			...
			if (type == InfinityType.Vertical) {
				...
				{
					index = (int)(content.anchoredPosition.y / (itemSize+itemSpace));
				}
				else
				{
					index = (int)(-content.anchoredPosition.y / (itemSize+itemSpace));
		...
		private Vector3 GetLocationAppear(Vector2 initVec,int location){
			...
			if (type == InfinityType.Vertical) {
				if (verticalType == VerticalType.TopToBottom) {
					vec = new Vector3 (vec.x, (-itemSize * location)-(itemSpace*location)/* - itemSize / 2*/, 0);
				} else {
					vec = new Vector3 (vec.x, (itemSize * location)+(itemSpace*location) /*+ itemSize / 2*/, 0);
				}
			} else {
